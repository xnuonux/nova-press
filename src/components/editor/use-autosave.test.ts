...
describe("formatSavedLabel", () => {
  ...
  it("reads 'saved just now' within five seconds", () => {
    ...
    expect(formatSavedLabel("saved", savedAt, savedAt + 2_000)).toBe(
      "saved just now",
    );
  });
  ...
  it("counts seconds, then rolls into minutes", () => {
    ...
    expect(formatSavedLabel("saved", savedAt, savedAt + 30_000)).toBe(
      "saved 30s ago",
    );
    expect(formatSavedLabel("saved", savedAt, savedAt + 180_000)).toBe(
      "saved 3m ago",
    );
  });
});
